...
def index():
    """Main page for uploading and processing samples"""
    ...
    return render_template(
        "index.html", results=None, best_match=None, score=None, sample_plot=None
    )
...
def update_comment():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE microplastics SET Comment=? WHERE ID=?", (new_comment, material_id)
    )
    conn.commit()
    conn.close()
...
def delete_sample():
    ...
    if sample_id:
        ...
        plot_file_path = os.path.join(
            "app/sample_plots", f"sample_{sample_id}_with_match.png"
        )
    ...
    if sample_id and material_id and match_score:
        try:
            ...
            if cursor.rowcount > 0:
                ...
                flash(
                    f"Successfully updated best match to '{material_id}' for sample '{sample_id}' with {(match_score_float * 100):.2f}% similarity",
                    "success",
                )
            else:
...
def regenerate_sample_plot_with_new_match(sample_id, new_match_id):
    ...
    try:
        ...
        print(
            f"Successfully regenerated plot for sample {sample_id} with new match {new_match_id}"
        )
